...
(function () {
  ...
  + ".alp-section .alp-inner{pointer-events:auto;will-change:transform,opacity;width:100%;}"
  ...
  + ".alp-giant{font-weight:800;line-height:.98;letter-spacing:-.01em;text-transform:uppercase;font-size:clamp(2.6rem,6.2vw,5.6rem);}"
  ...
  /* floating service cards (Noomo paper cards) */
  + "#alp-svc-head{position:absolute;left:6vw;top:15vh;z-index:5;max-width:64vw;pointer-events:none;}"
  + "#alp-svc-side{position:absolute;right:6vw;top:17vh;z-index:5;width:min(21vw,300px);font-size:12.5px;line-height:1.7;color:rgba(255,255,255,.55);text-align:left;}"
  + "#alp-svc-meta{position:absolute;left:6vw;bottom:7vh;z-index:6;}"
  + ".alp-fcard{position:absolute;width:clamp(215px,16.5vw,290px);padding:18px 17px 15px;border-radius:10px;background:rgba(242,244,248,.93);color:#16181d;box-shadow:0 24px 70px rgba(0,0,0,.5),0 2px 8px rgba(0,0,0,.3);cursor:default;will-change:transform,opacity,filter;pointer-events:auto;}"
  + ".alp-fcard .alp-fnum{font-size:9px;letter-spacing:.22em;color:rgba(20,22,28,.45);margin-bottom:9px;display:flex;justify-content:space-between;}"
  + ".alp-fcard h3{font-size:14.5px;font-weight:700;letter-spacing:-.01em;line-height:1.25;}"
  + ".alp-fcard p{margin-top:7px;font-size:11.5px;line-height:1.55;color:rgba(20,22,28,.6);}"
  + ".alp-fcard .alp-fin{transition:transform .4s cubic-bezier(.2,.8,.2,1);}"
  ...
  +   "#alp-svc-cards{position:absolute;left:22px;right:22px;top:36vh;display:grid;grid-template-columns:1fr 1fr;gap:8px;}"
  +   ".alp-fcard{position:relative!important;left:auto!important;top:auto!important;width:100%!important;padding:12px;filter:none!important;}"
  +   ".alp-fcard p{display:none;}"
  ...
  var SERVICES = [
    ...
    ["Brake Services", "Inspection, repair and replacement of pads, rotors and fluid."],
    ["Engine Diagnostics & Repair", "Identifying and fixing engine issues, including tune-ups."],
    ["Transmission Services", "Fluid changes, repair and replacement."],
    ["Suspension & Steering", "Shocks, struts and steering components."],
    ["Exhaust System Repairs", "Mufflers, catalytic converters and exhaust pipes."],
    ["Auto Electrical", "Battery test & replacement, alternators, fault-finding."],
    ["WOF Repairs", "We fix every fail — even though we don't do the test itself."]
  ];
  /* card scatter: [left%, top%, depth(0 back..1 front), rot(deg), bobDur(s)] */
  var CARD_POS = [
    [47, 14, 0.9, -2.5, 7.2], [66.5, 9, 0.5, 2, 8.4], [84, 17, 0.35, -1.5, 9.1],
    [40, 60, 0.45, 2, 8.8], [57, 52, 0.85, -2, 7.6], [75, 61, 0.55, 2.5, 8.1],
    [9, 64, 0.4, -2, 9.4], [25.5, 33, 0.3, 1.5, 8.6]
  ];
  var svcCards = SERVICES.map(function (s, i) {
    var c = CARD_POS[i];
    var z = c[2] >= 0.7 ? 6 : 4;
    var blur = c[2] >= 0.7 ? 0 : (0.7 - c[2]) * 6;
    return '<div class="alp-fcard" data-depth="' + c[2] + '" data-rot="' + c[3] + '" style="left:' + c[0] + 'vw;top:' + c[1] + 'vh;z-index:' + z + ';filter:blur(' + blur.toFixed(1) + 'px);">'
      + '<div class="alp-fin" style="animation:alp-bob ' + c[4] + 's ease-in-out infinite;">'
      + '<div class="alp-fnum"><span>SERVICE</span><span>0' + (i + 1) + ' / 08</span></div>'
      + "<h3>" + s[0] + "</h3><p>" + s[1] + "</p></div></div>";
  }).join("");
  ...
  var SEC = [
    ...
    { id: "services", stop: 30, enter: [0, -12], exit: [0, -12], svc: true, html:
      ...
      + '<div id="alp-svc-head"><div class="alp-meta alp-rise" style="margin-bottom:18px;"><span class="alp-chip">Workshop</span><span>Servicing / Repairs / Electrical</span></div>'
      + '<h2 class="alp-giant alp-split">Everything\nunder the hood —\nand around it.</h2></div>'
  ...
  var secKids = secEls.map(function (el) {
    return Array.prototype.slice.call(el.querySelectorAll(".alp-w, .alp-rise, .alp-fcard"));
  });
  ...
  root.addEventListener("touchmove", function (e) {
    ...
    e.preventDefault();
    if (transitioning || touchUsed || touchY === null) return;
  ...
  svcCardEls.forEach(function (card) {
    ...
    card.addEventListener("mousemove", function (e) {
      ...
      inner.style.transform = "perspective(700px) rotateY(" + (px * 10).toFixed(1) + "deg) rotateX(" + (-py * 10).toFixed(1) + "deg) scale(1.06)";
      card.style.zIndex = 9; card.style.filter = "blur(0px)";
    });
    card.addEventListener("mouseleave", function () {
      inner.style.transform = "";
      var d = parseFloat(card.getAttribute("data-depth"));
      card.style.zIndex = d >= 0.7 ? 6 : 4;
      card.style.filter = d >= 0.7 ? "blur(0px)" : "blur(" + ((0.7 - d) * 6).toFixed(1) + "px)";
    });
  });
  ...
  function styleSectionState(i, enterQ, exitQ) {
    ...
    if (n) {
      ...
      for (var k = 0; k < n; k++) {
        ...
        var ks = kids[k].style;
        var isCard = kids[k].classList.contains("alp-fcard");
        if (isCard) {
          var rot = parseFloat(kids[k].getAttribute("data-rot")) || 0;
          var depth = parseFloat(kids[k].getAttribute("data-depth")) || 0.5;
          var pax = (mx - 0.5) * depth * -26;
          var pay = (my - 0.5) * depth * -16;
          ks.opacity = r.toFixed(3);
          ks.transform = "translate(" + ((1 - r) * 40 + pax).toFixed(1) + "px," + ((1 - r) * 30 + pay).toFixed(1) + "px) rotate(" + (rot * r).toFixed(2) + "deg) scale(" + (0.92 + r * 0.08).toFixed(3) + ")";
        } else {
          var kdx = s.enter[0] !== 0 ? (s.enter[0] > 0 ? 1 : -1) * (1 - r) * 1.6 : 0;
          var kdy = s.enter[1] !== 0 ? (1 - r) * 0.9 : (1 - r) * 0.9;
          ks.opacity = r.toFixed(3);
  ...
  function render(p, force) {
    ...
    for (var i = 0; i < SEC.length; i++) {
      if (transitioning) {
        ...
        else if (secEls[i].style.visibility !== "hidden") { secEls[i].style.opacity = 0; secEls[i].style.visibility = "hidden"; }
      } else if (i === cur && MODE === "story") {
        styleSectionState(i, 1, 0);
      } else if (secEls[i].style.visibility !== "hidden") {
        secEls[i].style.opacity = 0; secEls[i].style.visibility = "hidden";
      }
    }
  ...
  (function tick(ts) {
    ...
    try {
      ...
      if (transitioning) {
        ...
        needs = true; // parallax cards track the mouse while parked
      }
